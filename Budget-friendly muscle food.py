...
def divide_values(BMR):
    ...
    min_Protein = round(((BMR * 0.30)/4),2)
    min_fats = round(((BMR * 0.35)/9),2)
    min_Crabs = round(((BMR * 0.35)/4),2)
    
    return min_Protein, min_fats, min_Crabs
...
def prepare_data(result_BMR_withGoal,min_Protein,min_fats,min_Carb):
    ...
    user_df = pd.DataFrame(
            [
                ("MinimumNUTR",result_BMR_withGoal,result_BMR_withGoal+250, min_Protein,min_fats,min_Carb),
            ],
            columns=["NUTR", "min_calories","max_calories","min_protein","min_fat","min_carbs"],
        ).set_index("NUTR")
...
def GenAMPL():
    ...
    # result_BMR_withGoal = BulkOrCut(result_BMR,"cutting")
    ...
    ampl.eval(
        ...
        param max_calories{NUTR};
        param min_protein{NUTR};
        param min_fat{NUTR};
        param min_carbs{NUTR};

        var Quantity{FOODS} integer >= 0;
        var Calories_gained = sum{f in FOODS} calories[f] * Quantity[f];
        var Protein_gained = sum{f in FOODS} protein[f] * Quantity[f];
        var Fat_gained = sum{f in FOODS} fat[f] * Quantity[f];
        var Carbohydrates_gained = sum{f in FOODS} carbs[f] * Quantity[f];
        ...
        subject to MaxCalories{i in NUTR}: sum{f in FOODS} (calories[f] * Quantity[f]) <= max_calories[i];
